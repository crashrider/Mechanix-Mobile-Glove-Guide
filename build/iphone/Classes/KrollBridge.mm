...
@implementation MechanixMobileGloveGuideObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mechanixmobilegloveguide);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any MechanixMobileGloveGuide patches in JS before a context is loaded... nice for 
...
{
	// create MechanixMobileGloveGuide global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mechanixmobilegloveguide = [[MechanixMobileGloveGuideObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mechanixmobilegloveguide];
	
	NSString *_mechanixmobilegloveguideNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mechanixmobilegloveguideNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mechanixmobilegloveguide valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.mechanixmobilegloveguide.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
